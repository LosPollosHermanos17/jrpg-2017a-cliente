...
public class ComandoConfirmarIntercambio extends Comando implements Serializable {
	...
	public ComandoConfirmarIntercambio(int idPersonajeSolicitante, int idPersonajeSolicitado,
			int idItemPersonajeSolicitante, int idItemPersonajeSolicitado) {
	...
	public void resolver(Object argumento) {
		...
		// Dentro del metodo mostrar que llaman los personajes intervinientes,
		// se llama al comando
		// ingresarMercado que ya se encarga de avisarle a los demas que los
		// vuelvan a agregar en la lista
			...
			if (idItemPersonajeSolicitante != 0 && idItemPersonajeSolicitado != 0) {
				...
				try {
					...
					JOptionPane.showMessageDialog(
							escuchaMensaje.getJuego().getEstadoJuego().getMenuMercado().getVentanaMercado(),
							"¡Has realizado el intercambio con exito!", "Intercambio exitoso",
				...
				JOptionPane.showMessageDialog(
						escuchaMensaje.getJuego().getEstadoJuego().getMenuMercado().getVentanaMercado(),
						"<html><i>" + personajeSolicitado.getNombre()
								+ "</i> ha rechazado la solicitud de intercambio.</html>",
			...
			if (escuchaMensaje.getJuego().getPersonaje().getId() == idPersonajeSolicitado) {
				...
				if (idItemPersonajeSolicitante != 0 && idItemPersonajeSolicitado != 0) {
					...
					try {
						...
						escuchaMensaje.getJuego().getCliente()
								.enviarComando(new ComandoActualizarPersonaje(personajeSolicitadoActualizado));
				...
				// Si no soy el personaje solicitante ni el solicitado, debo
				// actualizarlos en la lista
